...
export default function SessionCoachPanel({ language, summary, onEditCard, onStartRepair }: Props) {
  ...
  return (
    <div className="mt-5 rounded-ds-2xl border border-[#18181b] bg-[#0c0c0c] p-4 text-left shadow-card">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-ds-xl border border-amber-300/25 bg-amber-400/10 text-amber-100">
      ...
      <div className="mt-4 grid grid-cols-4 gap-2">
        <div className="rounded-ds-xl border border-[#18181b] bg-[#0a0a0a] p-3 text-center">
        ...
        <div className="rounded-ds-xl border border-[#18181b] bg-[#0a0a0a] p-3 text-center">
          <div className={`font-mono text-base font-bold ${summary.slowReviewCount > 0 ? 'text-amber-200' : 'text-white/50'}`}>
            {summary.slowReviewCount}
          </div>
          <div className="mt-0.5 text-[10px] uppercase tracking-wide text-white/40">
            {isDE ? 'Langsam' : 'Slow'}
          </div>
      ...
      {summary.problemCards.length > 0 && (
        <div className="mt-4 space-y-2">
          {summary.problemCards.map(problem => (
            <div key={problem.card.id} className="flex items-center gap-2 rounded-ds-xl border border-[#18181b] bg-[#0a0a0a] px-3 py-2">
              ...
              {onEditCard && (
                <button
                  ...
                  className="inline-flex h-9 shrink-0 items-center justify-center rounded-ds-xl border border-[#18181b] bg-[#0c0c0c] px-2 text-[11px] font-semibold text-white/55 transition hover:border-[#3f3f46] hover:text-white"
