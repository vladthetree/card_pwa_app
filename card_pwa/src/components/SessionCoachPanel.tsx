...
export default function SessionCoachPanel({ language, summary, onEditCard }: Props) {
  ...
  return (
    <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.035] p-4 text-left">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-amber-300/25 bg-amber-400/10 text-amber-100">
      ...
      {summary.problemCards.length > 0 && (
        <div className="mt-4 space-y-2">
          {summary.problemCards.slice(0, 3).map(problem => (
            <div key={problem.card.id} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/25 px-3 py-2">
              ...
              {onEditCard && (
                <button
                  ...
                  className="inline-flex h-9 shrink-0 items-center justify-center rounded-xl border border-white/15 px-2 text-[11px] font-semibold text-white/55 transition hover:border-white/30 hover:text-white"
