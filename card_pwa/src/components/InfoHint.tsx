/**
 * AI_CONTEXT: Reusable React component for info Hint; contributes to the card-learning UI and shared app interactions.
 */
...
export function InfoHint({ label, text }: InfoHintProps) {
  ...
  return (
    <span className="relative inline-flex items-center">
      ...
      <span
        className={`pointer-events-none absolute z-20 left-1/2 top-[calc(100%+8px)] -translate-x-1/2 min-w-56 max-w-72 rounded-ds-xl border border-[#18181b] bg-[#0c0c0c] px-3 py-2 text-[11px] leading-relaxed text-zinc-300 shadow-menu transition-all duration-200 ease-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}
      >
        {text}
      </span>
    </span>
