...
**Dreizehnte Runde am 2026-07-20** (Working Tree; 857/857 Tests + Build grün, Typecheck sauber): **Akronym-Fragen und Labs-Entscheidungs-Interaktion umgesetzt.**

- *Akronyme:* neuer Home-Modus „Akronyme" (`AcronymPracticeView`, verdrahtet in `HomeView`/`HomeDeckToolbar`/`HomeBottomBar`). `validate.mjs` generiert jetzt zusätzlich `src/data/sy0701Acronyms.ts` (alle 336 Crosswalk-Paare). `utils/acronymQuiz.ts` baut daraus deterministisch (mulberry32-Seed) Erkennungsfragen „Wofür steht X?" mit 4 Optionen; Distraktoren schließen strukturell dieselbe Abkürzung aus, damit mehrdeutige Abkürzungen nie zwei gültige Antworten in einer Frage haben. Die 11 mehrdeutigen Bedeutungspaare (MAC ×3, PAM, RA, RBAC, SAN) tragen einen handverfassten Disambiguierungs-Hinweis. Bewusst nur Erkennungsrichtung, keine Anwendungsfragen, keine Persistenz — reine Selbsteinschätzung wie der Video-Recall-Check. 10 Unit-Tests (`acronym-quiz.test.ts`) + 5 Komponententests, im Browser mit dem Puppeteer-Driver end-to-end verifiziert (richtig/falsch-Feedback, Rundenabschluss).
- *Labs:* neue `decision`-Interaktion (`LabDecisionInteraction`, single/multiple) ergänzt `matching`/`ordering` in `labScenarios.ts`, `LabScenarioView.tsx` und der `§13.2`-Normalisierung (`labSnapshot.ts`: neue `kind: 'decision'`/`comparison: 'set-equal'`-Zweige). Neue reine Scoring-Funktion `computeDecisionScore` (Treffer minus Fehlgriffe, nie unter 0) mit 9 Unit-Tests, wiederverwendet in `labSnapshot.ts` für konsistentes Scoring. 10 neue Szenarien verfasst, je im Original-Messer-Transkript verankert und mit `requirementIds` aus dem Leaf-Mapping versehen: Log-Analyse ×2, Firewall/ACL ×2, IAM ×2, Härtung ×2, Incident Response ×2 — inkl. öffentlicher Quellenangaben (NIST SP 800-92/94/61r2, Cisco ACL, Palo Alto, CIS Benchmarks) in `LAB_SCENARIO_SOURCE_REFS`. `LAB_TARGET_INVENTORY` 100 → 110. Im Browser verifiziert: Single-Select (Ransomware-Sofortmaßnahme) und Multi-Select (Chain of Custody) lösen korrekt bis „Geschafft!".
- Bewusst nicht angefasst: `requirementIds`-Backfill für die bestehenden 100 Matching/Ordering-Szenarien, fachliche Prüfung der 23 PBQ-artigen Karten — beide separat in OFFENE-PUNKTE.md #14 vermerkt.

**Vierzehnte Runde am 2026-07-21** (Working Tree; 860 Client-Tests + 81 Server-Tests + Build grün, Typecheck sauber, auf dem Pi deployt): **Prüfungstermin (`examDateIso`) synct jetzt geräteübergreifend mit dem Server-Profil.** Vorher nur `localStorage`, nie synced. Neuer Op-Typ `examDate.upsert` nach dem `videoNote.upsert`-Muster, aber scope- statt `[profileId+objective]`-Key, da es genau ein Skalar je Account ist:

- *Client:* `syncQueue.ts` (`SyncOperationType` + Deck-Filter-Ausnahme wie bei videoNote), `SettingsContext.tsx` (neues Feld `examDateUpdatedAt` als LWW-Basis, `setExamDateIso` pusht jetzt via `enqueueSyncOperation`), `syncPull.ts` (`applyExamDateUpsert` — da Settings `localStorage`-basiert sind statt Dexie, kein `db.put`, sondern direkter Blob-Read/Write plus neues `EXAM_DATE_SYNCED_EVENT` [appIdentity.ts], das ein gemounteter `SettingsProvider` per `useEffect` konsumiert, um sein React-State ohne Reload nachzuziehen — Settings hat keine Dexie-liveQuery-Reaktivität).
- *Server:* neue Tabelle `server_profile_settings` (PK `user_id`, ein Skalar-Feld `exam_date_iso`) in `schema.py` (inkl. `rebuild_server_state`-Wipe-Zeile), neuer `elif op_type == "examDate.upsert"`-Zweig in `operations.py` mit demselben `lww_should_apply` wie alle anderen Typen. Kein Whitelist-Gate in `routes/sync.py` nötig (Push-Route ist op_type-generisch).
- *Bewusst nicht angefasst:* der `/sync/snapshot`-Vollbootstrap-Pfad (für brandneu gepairte Geräte) — der ist Worker-basiert normalisiert (`snapshot-normalizer.worker.ts`) und für ein einzelnes Skalarfeld unverhältnismäßig aufwendig; ein neu gepairtes Gerät bekommt den Termin erst mit der nächsten Änderung. Bei Bedarf nachrüstbar nach demselben Muster wie `videoNotes` in der Snapshot-Response.
- *Deploy:* `sync.db`-Backup vor dem Schema-Change (`backups/sync.db.before-examdate-schema-*`), `card-sync-server.service` neu gestartet (`SYNC_REBUILD_ON_START=0` bestätigt, kein Rebuild ausgelöst), neue Tabelle live verifiziert, unauthentifizierte Push/Pull-Proben liefern weiterhin korrekt 401 (Auth-Gate unangetastet). `card-pwa-prod.service` braucht keinen Neustart (liest `dist/` pro Request neu, mit aktuellem Build-Hash verifiziert). Push-Pfad im Browser-Driver bis in die Sync-Outbox (IndexedDB) verifiziert.

**Nächste Schritte in Reihenfolge:**

1. `requirementIds`-Backfill für die bestehenden 100 Matching/Ordering-Lab-Szenarien; fachliche Prüfung der 23 PBQ-artigen Karten.
2. Software-Rest (klein): Reife-Beschriftung aus der UI nehmen (Zeile „Reife: …" in Lerneinheiten-Modus/Kachel), Phase-6-Feinschliff (Filter, Tastatur/Screenreader, kein horizontaler Overflow); optional Phase 7 (Komfort-Sync von Video-/Unit-Zustand).
...
- [ ] Alle 23 parsererkannten PBQ-artigen Karten sowie alle Labs mit dem tatsächlichen Kartenparser inventarisieren, fachlich prüfen und Objective-/Requirement-IDs zuordnen. *(teilweise: Inventar mit echtem Parser in generated/pbq-lab-coverage.json; `requirementIds` im generierten `sy0701ContentMap.ts` werden aus dem kuratierten Leaf-Mapping zurückgespiegelt; §13.1-Aufgabentypen über Matching/Ordering hinaus [Logs, Firewall/ACL, IAM, Härtung, IR] am 2026-07-20 mit 10 neuen `decision`-Szenarien samt `requirementIds` umgesetzt — weiter offen: `requirementIds`-Backfill für die bestehenden 100 Matching/Ordering-Szenarien und fachliche Prüfung der 23 PBQ-Karten)*
